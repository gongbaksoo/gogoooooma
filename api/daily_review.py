...
NEEDED_COLS = ["일별", "채널구분", "거래처명", "품목코드", "품목명[규격]", "판매액"]
...
def get_daily_review_summary(
    ...
    for c in [x["name"] for x in channels if x["group"] == "B"]:
        ...
        if len(gaps) >= 3:
            thr = max(float(np.median(gaps)) * 2, float(np.percentile(gaps, 90)))
            if ev["days_since"] > thr:
                ev["kind"] = "check"
                ev["message"] = (f"계상 지연 — 최근 12개월 통상 간격({np.median(gaps):.0f}일) 대비 "
                                 f"{ev['days_since']}일 경과.")
        b_events.append(ev)
    b_events.sort(key=lambda e: (e["kind"] != "check", -abs(e["today_net"])))
