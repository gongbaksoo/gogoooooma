...
3-1. **주요 거래처 감시** (48회차, **기본 접힘**) — A군 채널 소속 거래처 top-8의 당일 순매출 vs 같은 요일 8주 범위 위치. 월 리뷰가 거래처(11번가·지마켓·제제스스·롯데마트·이마트·CJ)를 상시 파는 것을 일 단위로. 표시명은 별칭(`11st→11번가`), 감시·매칭은 R열 원본. 예외 카드로 오른 거래처는 여기서도 강조·연결. 접힘 기본이라 조용한 날 화면 길이 불변.
...
| `components/daily-review/VendorWatch.tsx` | 145 | 주요 거래처 감시(A군 top-8, **기본 접힘**). 별칭 표시, 예외 거래처 강조 (48회차) |
