...
- 카드 3: **일 리뷰** → `/daily-review` ⭐ NEW (46회차)
- 세 카드 동일한 ghost 스타일 — 헤딩 22px/700, 본문 15px/400, 더보기 ghost CTA
...
#### 업로드 시각 표시 — 한국시(KST) 고정 (2026-06-15 39회차)
- `FileSelector`의 "저장된 파일" 목록 시각 포맷터 `formatDate()`는 `toLocaleString("ko-KR", { timeZone: "Asia/Seoul", ... })`로 **KST 고정**.
- `timeZone` 옵션을 생략하면 렌더 환경의 로컬 타임존을 따라가, Vercel(UTC) 등에서 렌더 시 한국시와 9시간(하루) 어긋난다. 모든 사용자 노출 시각은 렌더 위치와 무관하게 KST로 고정한다. 상세: `docs/error.md §54`.
...
PPT slide 3 "매출 리뷰 - 주요채널 이슈"를 채널 그룹 단위 동적 컴포넌트로 구현. **그룹 N행 × 2 차트** 기본 구성, 사용자가 그룹 수·이름·소속 채널 모두 편집 가능.

**구조 (N그룹 × 2 차트)** — 33회차에 컬럼형 → 행형으로 변경:
- 헤더: 좌 "주요 채널 이슈" + 우측 상단 `그룹 설정 ▾` 버튼 (모달 트리거)
- **그룹 세로 스택**(`flex flex-col gap-8`) — 그룹마다 전체 너비 1행:
  - 그룹 라벨(전체 너비, `그룹 이름` text + 채널 미지정 경고)
  - 그 아래 **2열 그리드**(`lg:grid-cols-2`, 모바일은 1열):
    - **좌측 차트** = 거래처별 매출 트렌드 (R열 거래처명, 그룹 내 채널 필터 집계)
    - **우측 차트** = 브랜드별 매출 트렌드 (D열 `품목그룹1`, 그룹 내 채널 필터 집계)
- 각 차트는 자체 `표시 거래처/브랜드 수정 ▾` 버튼 → ProductSelectionModal 재사용
- (이전 19~32회차: 그룹=N컬럼 grid, 컬럼 안에 두 차트 세로 쌓기. 33회차에 사용자 요청으로 그룹=행, 그룹 안 두 차트=가로 2열로 전치.)
...
#### 2.3.3.24 브랜드 상세 — 헤더 실적 요약(전체+주요 상품) + 섹션 전체 13개월 (2026-05-24 31회차)

"브랜드 종합"의 요약 라인(§2.3.3.23)을 **"브랜드 상세" 섹션의 각 브랜드(마이비/누비/쏭레브) 헤더 아래**에도 추가. 단, 브랜드별 목표가 없으므로 **목표비 제외**(전월비/직전3개월비/전년비만).

**요약 구성 (브랜드 카드 1개당):**
- **브랜드 전체 1줄** (백만): `실적 : N 백만 (전월비 ▲ , 직전 3개월비 ▲ , 전년비 ▲)` — 헤더에 브랜드명이 있어 「종합」처럼 이름 prefix 없음. 데이터 = 종합 트렌드(chart10/12/14).
- **선택 주요 상품별 N줄** (만원): `{상품명} 실적 : M 만원 (전월비 , 직전 3개월비 , 전년비)` — 「주요 상품 라인」에서 사용자가 선택한 상품(`selection.mainLine`) 순서대로. 데이터 = `brand_products[brand]`. 상품 매출은 소액이라 차트(y축)와 동일하게 **만원 단위**.
- 계산은 모두 프론트(`BrandSection`의 `summarize()`)가 13개월 시계열로: 실적=`vals[마지막]`, 전월비=`vals[-2]`, 직전3개월비=`avg(vals[-2..-4])`, 전년비=`vals[0]`(첫 점=전년 동월).
- **간격**: 브랜드 전체 1줄과 상품 요약 묶음 사이는 한 줄 간격(`mt-3`)으로 구분, 상품 줄끼리는 촘촘히(`space-y-0.5`).

**섹션 전체 13개월 확장:** 전년비 계산을 위해 브랜드 상세의 모든 차트(종합 트렌드 + 주요 상품 라인 + 개별 상품)를 **대상월-12 ~ 대상월(13개월)**로 통일 → 좌우 차트 x축 정렬 일치.
- 백엔드: `_brand_total_chart` `last12`→`last13`, `brand_products` 값 `last13_yymm`, 반환 `brand_products_months`=`last13_labels`. (채널 이슈 섹션의 12개월은 **그대로 유지** — `last12_yymm`/`last12_labels` 별도 보존)
- 프론트: 종합 트렌드 라벨 "12개월"→"13개월".

**개별 상품 차트 단위 라벨 추가 (2026-05-25 34회차):** 종합 트렌드는 `13개월 (단위: 백만)`(`toMillion`)인데 개별 상품 카드는 단위 라벨이 없어, 동일 백만 단위로 오인하면 "상품 매출 > 브랜드 종합"처럼 보이는 정합성 착시 발생(사용자 제보: 누비 종합 1백만 vs 스텐물병 차트 "4"). 개별 상품은 실제로 `toManWon`(÷10,000) **만원 단위**이므로 카드 제목 아래에 `13개월 (단위: 만)` 라벨을 명시(프론트 전용, 데이터·계산 무변경). 단위 차이로 인한 착시 방지. (주요 상품 라인 카드는 이번엔 미적용 — 후속 검토) → `docs/error.md §49`.

**배포**: 백엔드 변경 포함 → **Mac Mini 재배포 필요**. (프론트만 변경한 상품 요약 추가분은 Vercel 자동 배포)

#### 2.3.3.25 채널 종합 — 선택 채널별 실적 요약 라인 + 트렌드 13개월 (2026-05-24 32회차)

"브랜드 종합/상세"의 요약 라인을 **"채널 종합"(ChannelSection) 헤더 아래**에도 추가. 채널별 목표가 없으므로 **목표비 제외**(전월비/직전3개월비/전년비만).

**형식 (선택 채널당 1줄, 백만):** `{채널} 실적 : N 백만 (전월비 ▲ , 직전 3개월비 ▲ , 전년비 ▲)`
- 표시 채널 = `channelSelections[part]`(사용자 선택) 순서. 데이터 = 채널 옵션의 `values13`(13개월).
- 계산은 프론트(`ChannelSection`)가: 실적=`vals[마지막]`(당월), 전월비=`vals[-2]`, 직전3개월비=`avg(vals[-2..-4])`, 전년비=`vals[0]`(첫 점=전년 동월).

**트렌드 13개월 / 비중·월평균 12개월 분리:**
- 전년비를 위해 **채널별 매출 트렌드 차트만 13개월**(대상월-12~대상월)로 확장. 라벨 "최근 12개월"→"최근 13개월".
- **채널별 매출 비중 파이·월 평균 대비 실적 막대는 12개월 유지**(비중·월평균의 "최근 12개월" 의미 보존).
- 백엔드 `_option_from_frame`에 **`values13`(13개월) 필드 신규 추가**(기존 `values` 12개월은 비중·월평균용으로 유지), 응답에 **`channel_months13`**(=`last13_labels`) 추가. 채널 옵션 집계는 `groupby("월구분")` 1회 후 12·13개월 슬라이스(고유값×기간 반복 마스킹 회피, error.md §37).
- 프론트 폴백: `values13`/`channel_months13`이 없으면(백엔드 미배포) `values`/`channel_months`(12개월)로 동작 → 전년비만 `-`.
...
#### 2.3.3.27 AI 매출 분석 — 브랜드별 분석에 '주요 상품 매출'(대상월 매출 상위) 추가 (2026-06-16 43회차)

§2.3.3.26의 브랜드별 분석(섹션5)은 [브랜드 포커스](실적·채널)와 [브랜드별 상품 전월비](movers)만 근거로 삼아 브랜드의 **매출 상위 상품**을 직접 보여주지 못했다. 전월비 movers는 '많이 움직인 상품'이라 변동이 작은 꾸준한 주력 상품이 누락된다(예: 마이비 순한라인·삶기세제). "해당 브랜드 주요 상품 매출"을 보이게 하기 위해 컨텍스트 블록 1개 추가 + 섹션5 프롬프트 보강. **백엔드 변경 → Mac Mini 재배포 필요**.

**컨텍스트 — [브랜드별 주요 상품] 블록 신설 (`_build_analysis_context`)**
- `summary["brand_products"]`(이미 13개월 values 보유 → **`get_summary` 무수정**)에서 브랜드별 `values[-1]`(대상월 매출) 내림차순 **top5**(0원 제외), `직전월`(`values[-2]`) 동반. part=all 전용.
- 출력 예: `- 마이비: 얼룩제거제 82백만원(직전월 204백만원), 순한라인 25백만원(직전월 36백만원), …`
- 기존 [브랜드별 상품 전월비](movers) 블록은 유지 → 컨텍스트에 **매출순 + 전월비** 병존.

**프롬프트 섹션5 보강 (`analysis_prompts.json["all"]`)**
- 헤더 허용 블록에 `[브랜드별 주요 상품]` 추가. 데이터 규칙에 브랜드 상품 출처 명시(매출 상위 = [브랜드별 주요 상품], 전월비 급변 = [브랜드별 상품 전월비], 둘 다 대상월·직전월만·전년 수치 없음).
- 섹션5에 `· 주요 상품` 불릿 신설 — [브랜드별 주요 상품]의 대상월 매출 상위 상품을 이름·매출·직전월 대비로, 큰 변동은 [브랜드별 상품 전월비]로 보완. 실적·주요채널 불릿엔 `(출처: [브랜드 포커스])` 명시.
- 이커머스/오프라인은 브랜드별 섹션이 없어 무영향.

#### 2.3.3.28 AI 매출 분석 모달 — 분석 결과 복사 버튼 (2026-06-16 44회차)

회의 자료로 옮기기 쉽게, AI 분석 결과를 한 번에 클립보드로 복사하는 버튼을 `AIAnalysisModal.tsx`에 추가. **프론트 단독(Vercel 자동 배포)**.

- 결과 박스(`result`) 우상단에 "분석 결과" 라벨 + **복사** 버튼. 클릭 시 분석 텍스트 전체를 클립보드에 복사, 버튼이 "복사됨"으로 1.5초 표시 후 복귀(`copied` state + `setTimeout`).
...
#### 2.3.3.31 AI 매출 분석 — 현황 배경(파트별 저장) + AI 되묻기 (2026-07-18 52회차)

데이터만으로는 알 수 없는 배경(예: **쿠팡 사입 거래 종료 → 매출 0은 정상**)을 배경 없이 AI가 오독하는 문제 해소. 저장된 프롬프트(분석 지침)를 **1차 지침**으로 두고, 담당자가 붙이는 **현황 배경**을 부가 맥락으로 함께 전달. **백엔드 변경 포함 → Mac Mini 재배포 필요**(`docs/error.md §22`).

**현황 배경 (파트별, 백엔드 파일 저장)**
- 프롬프트와 **역할이 달라 별도 저장소** `analysis_contexts.json`(런타임, gitignore) — `{ all|ecommerce|offline: "..." }`. 프롬프트와 동일한 `_load_json_map` 공용 로더 사용.
- `GET /monthly-review/analysis-context/?part=` → `{ part, context }`, `POST` (body: `part`, `context`) → 저장.
- 모달 UI: `분석하기` 위 **"현황 배경 (선택)"** `textarea`(rows=3). **편집 모드와 무관하게 모든 사용자 입력 가능**(프롬프트=편집 모드 전용과 대비). 모달 열 때 파트별 저장값 프리필, `배경 저장` 버튼(변경 시에만 활성)으로 서버 저장.
- **저장하지 않아도** `분석하기` 시 현재 입력값을 `user_context`로 전송 → 이번 분석엔 즉시 반영(1회성 + 저장은 선택). 비우면 블록 자체를 안 넣어 **기존과 완전히 동일**하게 동작.

**프롬프트 조립 (`run_ai_analysis`)**
- `user_context`가 있으면 `[사용자 제공 배경]` 블록 주입 — "데이터에 안 드러나지만 신뢰할 맥락이니 해석에 반영. 단, 이 배경으로 **데이터에 없는 수치를 지어내지 말 것**"(grounding 유지).
...
### 2.4 일 리뷰 (`/daily-review`) ⭐ NEW (46회차)

> 기능 정의·데이터 근거·상수는 `project_plan.md §4.12`. 여기서는 **화면 규약**만 다룬다.

#### 2.4.0 이 화면의 제1원칙 — 거짓말하지 않기

이 페이지가 다루는 데이터는 ERP 매출계상일이라, **평범하게 만들면 반드시 사용자를 오해시킨다.**
그래서 정직성 장치를 UI 규약이 아니라 **응답 스키마 레벨**에 박았다.

| 사용자가 오해할 수 있는 것 | 화면이 막는 방법 |
|---|---|
| "어제 매출이 1,063만원이구나" | 제목이 **「직전 코어 계상일 계상 현황」**. 응답 키도 `sales`가 아니라 `accrual`. 각주 하드코딩(끌 수 없음) |
| "A군 + B군 = 전사겠지" | **3분해 표(A군 / B군 / 전사) 필수.** 2026-06-05엔 A군 순매출이 전사의 **395%**였다(반품이 B군에 몰려서) |
| "쿠팡이 어제 0원이니 망했다" | B군은 **금액 시계열이 응답 스키마에 없다.** 차트를 그리려야 그릴 수 없다 |
| "전일비 −30%다" | `dod`·`yoy` **필드를 만들지 않는다** (요일 효과 259배) |
| "착지 5억이겠구나" | `landing` **필드를 만들지 않는다** (DOM10 오차 31%) |
| "페이스 밴드 하회 = 위험" | **빨강 금지.** 밴드 위치 + base rate(평균 달성률 80.0%, 100% 달성 2/17) 병기 |
| "예외 0건 = 문제 없음" | **커버리지 한 줄을 접지 않고 노출** — "예외 판정 대상은 A군 6개 채널 = 전사 순매출의 39.2%" |
| "범위를 벗어났는데 왜 예외가 아니지?" | 범위 밖이지만 게이트 미만이면 **"이탈폭이 게이트(200만원) 미만 — 예외로 올리지 않음"** 을 명시. 강조색도 쓰지 않는다(`error.md §67`) |
| "11st가 뭐지 / 제제스스가 왜 없지" | 별칭으로 **사람이 아는 이름 표시**(`11st→11번가`, `스팜(제제지크)→제제스스`) + ERP 원본 병기. 별칭은 **표시 전용** — 집계·매칭은 R열 원본 exact(48회차) |
| "11번가 검색했더니 죽은 계좌" | 죽은 사입 계좌(`십일번가풀필먼트`)·중단된 `쿠팡(로켓)`은 **B군이라 A군 감시 패널·예외 카드에 애초에 안 뜬다**(48회차) |

#### 2.4.1 섹션 순서 (위계 = 읽어야 할 순서)

1. **데이터 가드** — 최종 계상일, 파일 기준일과의 지연, 잠정 배지, 오늘의 플래그(월말 마감 / 연휴·결측 직후 / 반품 배치일). **가장 흔한 이상은 매출이 아니라 업로드 누락이라 맨 위에 온다.**
2. **확인 필요 (예외 카드)** — 0건이면 한 줄로 끝난다. **판정일의 58.7%가 이 모습이다.**
3. **직전 코어 계상일 계상 현황** — A군/B군/전사 3분해 + A군 채널의 8주 범위 내 위치.
3-1. **심화 감시** (48~49회차, **기본 접힘**) — 월 리뷰가 상시 파는 3축을 일 단위로, 하나의 접이식 섹션에: **거래처**(A군 top-8, 별칭)·**브랜드**(마이비/누비/쏭레브 A군 일별, 쏭레브 '변동 큼')·**상품**(A군 SKU top-8, 품목명). 전 축 A군 스코프. 예외 카드로 오른 거래처는 강조·연결. 브랜드·상품은 **예외 카드로 올리지 않고 감시만**(상품 예외는 연 101건이라 카드화 시 원칙 위반). 접힘 기본이라 조용한 날 화면 길이 불변.
...
**AI 분석**은 섹션이 아니라 헤더의 **버튼 → 모달**이다(50회차). 페이지의 '조용한 날은 짧게' 원칙과 무충돌 — 사용자가 열 때만 나온다. 하드룰 14개가 서버에 고정돼 있어 LLM이 전일비·착지·배치해석·카드재탕·마크다운을 못 쓴다. 화면이 평문 렌더라 규칙 14(마크다운 금지)가 필수.

#### 2.4.2 차트 규약 — Recharts를 쓰지 않는다

- **시계열 차트가 하나도 없다.** B군 금액 추이는 금지(계상 타이밍 그래프가 됨)이고, A군도 "그날 값이 8주 범위 안 어디인가"만 필요하다.
- **범위 막대(RangeBar)는 순수 CSS.** 회색 트랙(8주 min~max) + 세로 마커(당일 값). 마커 색은 **예외로 flagged된 경우에만** 강조색(`#b45309`), 아니면 검정.
- → 기존 차트 컴포넌트 9곳에 하드코딩된 **월 형식(`YYYY-MM`) X축 포맷터 문제를 아예 우회**한다. 일 축 지원 차트를 새로 만들 필요가 없다.

#### 2.4.3 단위 — 만원 고정

- `frontend/src/lib/manwon.ts` — `toManwon` / `fmtManwon` / `fmtManwonBare` / `fmtEok`.
- **함수명에 단위를 박는다.** 기존 차트 8곳에 `toMan`이라는 같은 이름의 함수가 있는데 `ChannelIssueSection`만 ÷10,000이고 나머지는 ÷1,000,000이다(`error.md §45`). 일 값은 월 값의 약 1/30이라 백만원으로 반올림하면 0~수십으로 뭉개진다.

#### 2.4.4 컴포넌트

| 파일 | 줄 | 역할 |
|---|---|---|
| `app/daily-review/page.tsx` | 169 | 날짜 선택(임의 과거일 조회 가능) · pull 방식 로드 · `non_core_day` / `no_data` / 에러 분기 |
| `components/daily-review/types.ts` | 134 | API 응답 타입. **`dod`/`yoy`/`landing`이 없는 것은 실수가 아니다** |
| `components/daily-review/DataGuard.tsx` | 60 | 신선도 배너 + 플래그 배지 |
...
| `components/daily-review/DeepWatch.tsx` | 220 | 심화 감시 — 거래처·브랜드·상품 3축(전부 A군 스코프, **기본 접힘**). 별칭 표시, 예외 거래처 강조, 게이트미만·표본부족·변동큼 배지 (48~49회차, VendorWatch 흡수) |
...
| `components/daily-review/AIAnalysisModal.tsx` | 205 | AI 분석 모달(헤더 버튼으로 염). 지침은 편집 가능하나 **하드룰 14개는 서버 코드 고정**. 신호 없으면 "AI를 호출하지 않았습니다" 표기 (50회차) |

#### 2.4.5 상태 분기

| 상태 | 조건 | 화면 |
|---|---|---|
| `non_core_day` | 계상 채널 < 6 (주말·공휴일 등) | 계산하지 않고 거부 + **직전 계상일 바로가기 버튼**. "13,680원짜리 토요일"이 '특이사항 없음'을 띄우는 것을 막는다 |
| `month_end_batch` | 대상일 = 달력 말일 | 예외 판정 미실시 배너. 페이스는 밴드 대신 **달성률** |
| `post_gap` | 직전 코어일과 4일 이상 | 급증 판정 미실시 배너 |
| `return_batch` | \|반품\| ≥ 500만 AND ≥ 총매출의 50% | 반품 열 강조. A군/B군 분해로 오독 차단 |
| `stale` | 데이터가 대상일까지 미도착 | 페이스 지표 전면 억제 |
| `provisional` | 대상일이 최근 45일 이내 | "이 판정은 잠정" + 스냅샷 파일명 표기 |
