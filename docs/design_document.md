...
|-------|------|-----------------|-----|-----------|
| 4. 브랜드별 매출 트렌드 | LineChart (N-series, 12개월) | D열(품목그룹1) 동적 — 개별 브랜드(판매액 desc) + 기타. 기본 상위 3개 표시 (§2.3.3.22) | `YY.MM` | B-6 팔레트 (`getMultiSeriesStyle`) |
...
| 브랜드별 매출 트렌드 (chart4) | D열 동적(개별 브랜드+기타) — 30회차에 고정 5개에서 변경(§2.3.3.22) | chart4·chart5 **공유** | 있음 |
...
- 모달(`AIAnalysisModal.tsx`):
  ...
  - **데이터 없음(`summary == null`, 파일/대상월 미선택)**: 모달은 열리고 프롬프트 작성은 가능. `분석하기`만 비활성 + "매출·목표 파일과 대상 월을 먼저 선택하면 분석할 수 있습니다." 안내.
...
#### 2.3.3.21 품목 단위 차트 표시 단위 백만원 → 만원 (2026-05-24 30회차)

품목(S열)·거래처(R열)처럼 **월 매출이 100만원 미만인 항목**이 많은 차트들이, 변환 함수가 백만원 정수 반올림(`Math.round(v / 1_000_000)`)이라 그런 항목이 전부 0~1로 뭉개져 **실제 매출(예: 2in1 컵 2026-04 = 262,018원)이 차트에 0으로 표시**되는 문제 발생. 만원 단위(천원 자리 반올림, `Math.round(v / 10_000)`)로 변경하여 해상도 확보 → 262,018원 → **26만원**.

**적용 컴포넌트 (둘 다 품목 단위 라인을 그림):**

1. **`BrandSection.tsx` (사용자가 실제로 본 화면)** — 브랜드 상세의 "개별 상품" 소형 멀티플(차트 제목 = 품목명, 예: "2in1 컵") + "주요 상품 라인". 함수를 단위별로 분리: `toMillion`(÷1,000,000, **브랜드 종합 트렌드**=집계값) / `toManWon`(÷10,000, **품목 차트**). 품목 차트 툴팁 `백만`→`만원`. 브랜드 종합 트렌드(①)는 집계값이라 **백만 유지**(라벨 `12개월 (단위: 백만)`·툴팁 그대로).
2. **`ChannelIssueSection.tsx`** — "주요 채널 이슈"의 두 미니 차트(거래처별/브랜드별 매출 트렌드). `toMan`을 `Math.round(v / 10_000)`로 변경, 툴팁 `백만`→`만원`, 두 서브타이틀 `(백만)`→`(만원)`. (섹션 전체 만원 통일)

> ⚠️ 최초 30회차에는 차트 제목이 "2in1 컵"인 것을 보고 `ChannelIssueSection`(제목 고정 "거래처별/브랜드별 매출 트렌드")만 고쳤으나, **실사용 화면은 `BrandSection` "개별 상품"**(제목=품목명)이었음. 컴포넌트 오인 → 후속에서 BrandSection 보정(`docs/error.md §46`).

- **적용 범위**: 위 2개 컴포넌트의 품목 단위 라인 한정. 종합 9개 차트(§2.3.3.5 Tooltip 규약)·브랜드 종합 트렌드는 집계값이 커서 **백만 단위 유지**.
- **백엔드 무변경** — 백엔드는 원 단위 정확값을 반환(`brand_products["누비"]` "2in1 컵" 2026-04 = 262,018원 직접 검산 일치). 순수 프론트 표시 변환 → **재배포 불필요(Vercel 자동)**. 상세: `docs/error.md §45`·`§46`.

#### 2.3.3.22 브랜드별 차트 — 고정 5개 → D열 동적 전체 브랜드 (2026-05-24 30회차 후속)

브랜드별 매출 트렌드(chart4)·비중(chart5)이 `BRAND_MAIN = ["마이비","누비","쏭레브","에코보"] + 기타` **고정 5개**라, D열(품목그룹1)의 실제 브랜드(13개) 중 데일리케어(에코보보다 큼)·라포레띠 등이 "기타"에 묻혀 수정 모달에서도 5개만 보였음. 사용자 요청으로 **D열 실제 브랜드 전체를 동적 노출**하도록 변경.

**백엔드 (`monthly_review.py`):**
- `BRAND_ETC = ["기타(타사)", "부자재(공통)", "구브랜드"]` + 빈값(NaN/공백) → **"기타" 한 칸으로 묶음**.
- 그 외 모든 품목그룹1 = **개별 브랜드**, `groupby("품목그룹1")["판매액"].sum()` **내림차순** 정렬. 새 브랜드는 자동으로 개별 노출(BRAND_ETC만 하드코딩).
- `brand_names = brand_individual + ["기타"]`, chart4/chart5 동일. `brand_colors`는 placeholder(프론트가 무시).
- chart6(월 평균 대비 실적, 마이비/누비/쏭레브/마+누+쏭)은 **변경 없음**(별도 고정 비교군).
- 검증(260519.csv, part=all): 개별 9 + 기타 = **10 시리즈**, 판매액 desc = 마이비·누비·쏭레브·데일리케어·라포레띠·에코보·깜·스킨베리·제제지크.

**프론트:**
- `Chart4BrandTrend`/`Chart5BrandShare`는 이미 `series_names` 동적 + `getMultiSeriesStyle`(B-6 14슬롯) 사용 → **컴포넌트 변경 없음**(10개 색 모두 구분).
- 기본 표시 = **상위 3개**: `page.tsx`에서 `selected = overviewSelections.brand[part] ?? summary.chart4.series_names.slice(0, 3)`. 미편집 시 상위 3개(마이비/누비/쏭레브)만, 나머지는 수정 모달에서 추가. (`[]`=전부 해제는 그대로 빈 차트)

**배포**: 백엔드 변경 포함 → **Mac Mini 재배포 필요**(`docs/error.md §22`). 미반영 시 운영은 5개 유지.

#### 2.3.3.23 브랜드 종합 — 선택 브랜드별 실적 요약 라인 + 트렌드 13개월 + 브랜드 목표 (2026-05-24 30회차 후속)

"종합" 섹션의 실적 요약 라인(§2.3.3.18)을 **"브랜드 종합" 섹션 헤더 아래**에도 추가. 단, **선택해 둔 브랜드마다 한 줄씩(줄바꿈)**.

**형식 (브랜드당 1줄):** `{브랜드} 실적 : N 백만 (목표비 X% , 전월비 ▲Y% , 직전 3개월비 ▲Z% , 전년비 ▲W%)`
- 표시 브랜드 = chart4 선택(`overviewSelections.brand[part] ?? series_names.slice(0,3)`), 그 순서대로.
- 계산은 프론트(`BrandSummaryLines`)가 chart4(13개월) 데이터로: 실적=`vals[마지막]`(대상월), 전월비=`vals[-2]`, 직전3개월비=`avg(vals[-2..-4])`, 전년비=`vals[0]`(첫 점=전년 동월).

**전년비 → 브랜드 트렌드(chart4) 12→13개월:** 전년 동월을 포함시키려고 chart4를 **대상월-12 ~ 대상월(13개월)**로 확장. 백엔드 `_trailing_series(..., periods=last13)`. 차트 X축도 13개월로 보임. (chart5 비중 파이는 **최근 12개월 합계 유지** — `_share_pie`는 last12)

**목표비 → 브랜드별 목표 신규:**
- 원본 엑셀 '전사' 시트에 브랜드별 월별 목표(계획)가 있음(24년=실적만, 25·26년=월별 목표/실적/예상실적). 마이비/누비/쏭레브/데일리케어/에코보 행 × 월별 "목표" 컬럼을 추출 → **`api/brand_targets.csv`**(`월/브랜드/목표`, 25·26년 120행).
- 백엔드 `_load_brand_targets()` → summary `brand_targets`(대상월 브랜드별 목표). 목표비 = `실적 / 목표 × 100`.
- ⚠️ **제약**: ① 전사(전 파트) 목표라 **part=all에서만** 제공(이커머스/오프라인은 `{}` → 목표비 `-`). ② 목표가 있는 5개 브랜드만 목표비 표시, 그 외(라포레띠·제제지크·기타 등)는 `-`. ('전사'의 "기타"는 차트의 동적 "기타"와 구성이 달라 매핑 제외.)
- **파일 위치**: `brand_targets.csv`는 업로드 런타임 데이터가 아닌 추출 레퍼런스 → `uploads/`(gitignore) 아닌 **`api/`(추적 경로)**에 둬 git pull로 배포.

**배포**: 백엔드 변경 + 신규 데이터 파일 포함 → **Mac Mini 재배포 필요**(git pull로 `brand_targets.csv` 동반).
