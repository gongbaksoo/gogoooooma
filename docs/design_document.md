...
#### 2.3.3.21 주요 채널 이슈 — 표시 단위 백만원 → 만원 (2026-05-24 30회차)

채널 이슈 섹션의 두 미니 차트(거래처별/브랜드별 매출 트렌드)는 품목(S열)·거래처(R열)처럼 **월 매출이 100만원 미만인 항목**이 많은데, 변환 함수가 백만원 정수 반올림(`Math.round(v / 1_000_000)`)이라 그런 항목이 전부 0~1로 뭉개져 **실제 매출(예: 2in1 컵 2026-04 = 262,018원)이 차트에 0으로 표시**되는 문제 발생.

- **변경**: `ChannelIssueSection.tsx`의 `toMan`을 **만원 단위(천원 자리 반올림)** `Math.round(v / 10_000)`로 변경. 262,018원 → **26만원**. (함수명 `toMan`(만원)과 동작이 비로소 일치)
- 단위 라벨 동반 수정: 툴팁 `백만` → `만원`, 두 서브타이틀 `(백만)` → `(만원)`.
- **적용 범위**: 채널 이슈 섹션 한정. 종합 9개 차트(§2.3.3.5 Tooltip 규약)는 집계값이 커서 **백만 단위 유지**.
- **백엔드 무변경** — 백엔드는 원 단위 정확값을 반환(7개 채널 합산 검산 일치). 순수 프론트 표시 변환 문제 → **재배포 불필요(Vercel 자동)**. 상세: `docs/error.md §45`.
