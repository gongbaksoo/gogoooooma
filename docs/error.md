...
## 77. **엉뚱한 머신을 정리해 놓고 "해결됐다"고 보고했다** — 작업 머신 ≠ 운영 머신 (2026-09-16 55회차)

### 🚨 증상 / 함정
- 사용자 제보는 명확히 **"서버로 쓰는 맥미니에 파일이 쌓인다"**였다. 그런데 54회차에서 정리 스크립트를 돌린 곳은 **노트북(J-MacBook-Pro-M5)의 작업 복사본**이었고, "313MB → 168MB, 145.7MB 회수"를 **문제 해결로 보고**했다. 운영 서버는 손도 대지 않은 상태였다.
- **함정 1 (핵심) — 저장소가 그럴듯해서 의심하지 않았다.** 노트북 `api/metadata.db`에 169MB짜리 실제 업로드 데이터(`260519.csv` 등)가 들어 있었다. 운영 데이터처럼 보였기 때문에 "여기가 그 서버"라는 가정이 검증 없이 통과했다. **데이터가 진짜처럼 보이는 것은 머신이 맞다는 증거가 아니다.**
- **함정 2 — `hostname` 한 줄이면 끝날 확인을 안 했다.** 제보에 "맥미니"가 명시돼 있었는데도 현재 셸이 어느 머신인지 확인하지 않았다. 나중에 확인하니 `J-MacBook-Pro-M5.local`, 8000 포트 리슨 없음, `com.avk.backend` LaunchAgent 없음 — **3개 중 아무거나 하나만 봤어도** 즉시 드러났다.
- **함정 3 — 메모리의 옛 정보가 오답을 유도할 뻔했다.** 프로젝트 메모에 "노트북→맥미니 키 인증 미설정(publickey 거부)이라 비대화형 ssh 불가"(2026-05-21)라고 적혀 있었다. 이걸 그대로 믿었으면 "제가 할 수 없습니다"라고 답했겠지만, 실제로 시험해보니 `macmini-codex`(IdentityFile `id_ed25519`)로 **정상 인증**됐다. **메모에 적힌 환경 제약은 단정 전에 재시험해야 한다** — 환경은 메모보다 빨리 바뀐다.
- 실제 운영 서버 상태(나중에 실측): `api/uploads/` **6.7GB / 데이터 81건 / parquet 44건**. 노트북(313MB/9건)과 **규모가 20배 달랐다.** 숫자를 서로 대볼 생각만 했어도 이상함을 눈치챌 수 있었다.

### ✅ 조치
- `hostname` + `lsof -iTCP:8000` + LaunchAgent 확인으로 머신 정체를 확정한 뒤, 운영 저장소 경로(`/Users/j_mac_mini/Projects/AVK_Sales`)를 ssh로 실측.
- 사용자에게 **오보를 먼저 정정**하고, 운영 dry-run(삭제 76건/6.50GB)을 제시해 승인받은 뒤 배포·정리 수행.
- `project_plan.md §7`에 **「작업 머신 ≠ 운영 머신」 표 + 백엔드 배포 SOP(ssh)** 명문화. 프로젝트 메모의 ssh 제약 기술도 갱신.
- 결과: 6.7GB → 455MB, 디스크 여유 22GB → 28GB, 전 엔드포인트 정상.

### 💡 향후 권장
- **"서버/운영에서 X가 일어난다"는 제보를 받으면 첫 명령은 `hostname`이다.** 이어서 그 서비스가 이 머신에서 실제로 도는지(포트 리슨·프로세스·서비스 등록) 확인한다. 로컬 저장소에 그럴듯한 운영 데이터가 있어도 증거가 되지 않는다.
- **조치 결과를 보고할 때 "어느 머신에서" 측정한 값인지 함께 쓴다.** 머신을 생략한 용량·건수 보고는 그 자체로 오보의 씨앗이다.
- **제보 수치와 내 실측 수치의 자릿수가 다르면 대상을 잘못 잡은 것부터 의심한다** (여기선 6.7GB vs 313MB).
- **메모리·문서에 적힌 "불가능/미설정" 기술은 단정의 근거로 쓰기 전에 1회 재시험한다.** 특히 접근 권한·인증·툴체인처럼 사람이 나중에 설정했을 수 있는 항목. 재시험 결과가 다르면 메모를 즉시 갱신한다(§76의 OS 업그레이드 리셋과 반대 방향의 같은 문제).
- 배포는 코드 반영과 재시작이 **한 쌍**이다. push만으로 백엔드가 바뀐다고 말하지 말 것 — 이 저장소는 프론트만 자동 배포된다.

---

## 76. macOS 업그레이드가 Xcode 라이선스 동의를 초기화 → **모든 git 명령이 실행 전에 차단** (2026-09-16 54회차)

### 🚨 증상 / 함정
- 문서 수정을 마치고 커밋하려는데 `git status`·`git branch`·`git --version`까지 **전부 동일 메시지로 실패**:
  ```
  You have not agreed to the Xcode license agreements. Please run 'sudo xcodebuild -license'
  ```
- **함정 1 — git 저장소 문제로 오인하기 쉽다.** 에러가 git 명령에서 나오지만 원인은 저장소·권한·네트워크가 아니라 **Apple 개발자도구 라이선스 게이트**다. `/usr/bin/git`이 Xcode CLT 번들 소속이라, 동의 전에는 git 자체가 실행되지 못한다. 저장소를 진단하면 시간만 버린다.
- **함정 2 — 세션 사이에 OS가 바뀌었다.** 직전 세션 `Darwin 25.6.0` → 이번 세션 `Darwin 27.0.0`. macOS 메이저 업그레이드가 라이선스 동의를 리셋했다. **"어제 되던 게 오늘 안 되면 환경이 바뀌었는지부터"** 확인해야 한다(§1 TCC 회수와 같은 계열).
- **함정 3 — 우회로가 없었다.** `/opt/homebrew`는 존재했지만 **Homebrew git은 미설치**라 시스템에 git 바이너리가 `/usr/bin/git` 하나뿐이었다. 대체 바이너리 탐색(`find /usr/local /opt -name git -type f -perm -u+x`)으로 없음을 확인하고 즉시 사용자에게 넘긴 게 맞는 판단이었다.
- **함정 4 — 에이전트가 대신 해결할 수 없는 종류다.** `sudo` 비밀번호 + 약관 본인 동의가 필요하다. 첫 시도는 비밀번호 오입력(`Sorry, try again.`)으로 실패했고, 재실행 후 약관 표시 → `agree` 입력으로 통과.

### ✅ 조치
- 사용자가 터미널에서 직접 실행: `sudo xcodebuild -license` → enter로 약관 표시 → **`agree`** 입력.
- 통과 후 `git version 2.54.0 (Apple Git-157)` 정상 확인, 커밋·푸시 완료(`7ba8c73`).

### 💡 향후 권장
- **git 명령이 저장소와 무관한 메시지로 일괄 실패하면 원인을 저장소 밖에서 찾는다.** 특히 `git --version`처럼 저장소가 필요 없는 명령까지 같이 죽으면 **툴체인·라이선스·권한 게이트**가 원인이다.
- **작업 시작 시 OS 버전 변화를 체크 대상에 넣는다.** macOS 메이저 업그레이드는 라이선스 동의(§76)·TCC 권한(§1)·CLT 경로를 함께 리셋한다. 업그레이드 직후 첫 세션은 `git`·`python3`·venv가 살아 있는지 먼저 확인.
- **sudo·본인 동의가 필요한 조치는 추측으로 우회하지 말고 정확한 명령과 함께 사용자에게 넘긴다.** 대체 경로(다른 바이너리)가 있는지는 확인하되, 없으면 즉시 보고하는 편이 빠르다.
- (선택) Homebrew git(`brew install git`)을 두면 Apple git이 게이트에 걸려도 작업이 멈추지 않는다.

---

## 75. 정리 함수는 **있었지만 호출되지 않았다** — 이중 저장소에서 한쪽만 상한이 걸려 파일이 무한 누적 (2026-09-16 54회차)

### 🚨 증상 / 함정
- 제보: "맥미니에 파일이 계속 쌓인다." 실측 — DB `uploaded_files` **2건(169.6MB)** vs 디스크 `api/uploads/` **9건(313MB)**. 디스크 9건 중 **7건은 DB에 아예 없는 고아**였다.
- **함정 1 (핵심) — 코드는 이미 있었다.** `index.py`에 `cleanup_old_files(max_files=5)`가 멀쩡히 정의돼 있었지만, 호출부가 **DB 저장 실패 시 fallback 분기 안에만** 있었다. 정상 경로(DB 저장 성공)에서는 `cleanup_old_files_in_db()`만 돌아 **DB만 5개로 줄고 디스크는 아무 상한도 받지 않았다.** "정리 함수가 있다"는 것을 "정리되고 있다"로 읽으면 놓친다 — grep으로 **정의가 아니라 호출부**를 봐야 보인다.
- **함정 2 — 디스크 사본은 업로드가 아니라 조회가 만든다.** 업로드 엔드포인트만 읽으면 DB에만 쓰는 것처럼 보인다. 실제로는 조회 시점에 `ensure_file_on_disk()`가 DB BLOB을 디스크로 다시 써낸다. 게다가 **같은 로직이 `index.py`와 `monthly_review.py` 두 곳에 복제**돼 있어 한 파일만 보면 사본 생성 경로를 못 찾는다.
- **함정 3 — 상한 카운트에 `.gitkeep`이 섞였다.** `cleanup_old_files`는 최상위 **모든** 파일을 셌다. `max_files=5`가 실질 데이터 4개로 동작하고, 가장 오래된 `.gitkeep`이 삭제될 수도 있었다(= 추적 파일 유실).
- **함정 4 — 정렬 키가 `uploaded_at`이었다.** 같은 파일명 재업로드는 `updated_at`만 갱신한다. `uploaded_at` 기준으로 자르면 **자주 갱신하는 파일이 방치된 파일보다 먼저 삭제**된다.
- **함정 5 (수정 쪽의 함정) — "DB 미러링"은 그 자체로 위험하다.** 디스크를 DB에 맞추는 설계는 DB가 비면 **디스크 전량 삭제**가 된다. 이건 §44에서 실제로 났던 사고(빈 DB가 덮어써져 업로드 유실)와 정확히 같은 구조다. 서버를 `api/`가 아닌 루트에서 띄우면 `sqlite:///./metadata.db`가 **빈 DB를 새로 만든다**(실제로 루트에 0바이트 `metadata.db`가 있었다).
- 부수 누수: `DELETE /files/{filename}`이 메모리 캐시만 비우고 **parquet은 남겼다**(파일당 ~6MB 영구 잔존).

### ✅ 조치
- `api/retention.py` 신설 — DB·디스크·parquet 정리를 **한 함수로 통합**(`enforce_retention`). DB를 기준 원장으로 삼고 디스크를 미러링, 살아남은 해시가 아닌 parquet까지 회수. 대상은 `.csv`/`.xlsx`만이라 `.gitkeep`·하위 디렉토리는 불가침(함정 3 해소).
- **호출부를 전 경로에 연결** — 업로드(DB/fallback 공통)·파일 삭제·서버 기동 3곳. 목표 파일은 `enforce_target_retention()`으로 `monthly_review.py` 업로드에 연결.
- **정렬 키를 `updated_at`으로 통일** — `list_files_in_db()`·`cleanup_old_files_in_db()` 양쪽(함정 4 해소).
- **안전장치**: DB 목록이 비었거나 연결 불가면 미러링을 **중단**하고 디스크 mtime 기준으로만 자른다(`mode: "disk"`). 함정 5 차단.
- 죽어 있던 `cleanup_old_files()` 제거 — 경쟁하는 두 구현을 남기지 않는다.
- 삭제 전 복구 가능성 확인: 최신 고아 `260615.csv`는 프로젝트 루트 사본과 **SHA256 동일**함을 대조한 뒤 삭제(재업로드로 복구 가능).
- 결과: `api/uploads/` **313MB → 168MB**(145.7MB 회수), 9건 → 2건(= DB 보관분과 일치), parquet 2건 → 1건. 상세 설계는 `project_plan.md §5.5`.

### 💡 향후 권장
- **"정리/제한 로직이 있는가"가 아니라 "모든 경로에서 호출되는가"를 본다.** 정의만 grep하면 죽은 코드를 살아 있다고 읽는다. 조건 분기 안에 있는 정리 호출은 특히 의심 — 폴백 경로에만 걸려 있기 쉽다.
- **저장소가 둘이면 상한도 하나로 관리한다.** DB와 디스크가 각자 5개를 세면 실제 보관량은 둘의 합이 되고, 한쪽 상한이 죽으면 조용히 무한 누적된다. **한쪽을 기준 원장으로 정하고 나머지는 미러**로 두되, **기준이 비었을 때 미러가 전량 삭제되지 않도록 반드시 가드**를 건다(§44 사고와 동형).
- **누적 제보("파일이 계속 쌓인다")는 코드보다 실측을 먼저** — DB 건수 vs 디스크 건수 vs 캐시 건수를 나란히 세면 어느 층의 상한이 죽었는지 한 번에 드러난다.
- **파괴적 일괄 삭제는 dry-run → 복구 가능성 확인 → 실행** 순으로. 삭제 목록과 용량을 먼저 출력하고, 지울 파일의 사본·해시가 다른 곳에 있는지 대조한다.
- **상한 카운트 대상을 명시적으로 좁힌다** — 확장자 화이트리스트로 세지 않으면 `.gitkeep`·로그·임시파일이 슬롯을 먹거나 삭제된다.

---

## 74. html2canvas는 인라인 형광 배경을 위로 ~1px 올려 그린다 + 스크래치 재현 시 `tar` 추출 경로 충돌 (2026-07-18 53회차)

### 🚨 증상 / 함정
- **본체**: PDF(월 리뷰 `html2canvas`+`jspdf`)에서 형광펜이 "위치가 안 맞아" 보임. 추정만으로 보정값을 넣으면 틀리므로 **동일 폰트(Pretendard)·동일 `html2canvas@1.4.1`로 격리 재현 후 픽셀 측정**함 → `html2canvas`가 인라인 `background-color`를 실제 브라우저보다 **위로 ~1px 올려** 그려 **글자 하단(괄호·숫자·`_`)을 안 덮음**(형광 하단 여백 0px, 브라우저 ~0.4px). 육안 스크린샷으론 안 보였고 **캔버스 픽셀 스캔(형광 밴드 vs 검은 글자 상하 여백)**으로만 잡혔다.
- **함정 1(오답 후보)**: `line-height` 정규화안은 **PDF 줄간격까지 바꿔** 부적합, `inline-block`안은 **줄바꿈되는 형광을 한 덩어리로 깨뜨려** 부적합. box-decoration-break:clone **단독으론 무효**(위치 그대로).
- **함정 2(도구)**: 스크래치에서 `npm pack html2canvas` → `tar xzf`, 이어 `npm pack pretendard` → `tar xzf` 했더니 **둘 다 `package/`로 풀려** 뒤엣것이 앞엣것을 덮어써 `html2canvas.min.js`가 사라짐. `tar xzf a.tgz -C dirA` / `-C dirB`로 **분리 추출**해야 함.

### ✅ 조치
- 캡처 시점만 보정: `html2canvas` 옵션 `onclone(_doc, el)`에서 형광 span(`span[style*="background-color"]`)에 `box-decoration-break:clone`(+`-webkit-`) + `padding-bottom:1.5px`. 측정상 하단 여백 **0→1.5px**, 상단 불변, 2줄 형광 정상. **화면·줄간격 불변**(클론에만 적용). 상세 `design_document.md §2.3.3.32`.
- 신뢰도: `onclone` 2번째 인자가 캡처 대상 클론인지 라이브러리 소스로 확인(`onclone(documentClone, referenceElement)`). 새 lint/tsc 에러 0(HEAD 대비 stash 대조, §73 방식).

### 💡 향후 권장
- **"위치가 미세하게 안 맞다"류는 육안·스크린샷으로 단정하지 말고 캔버스 픽셀을 스캔해 수치화**한 뒤 고친다(오프셋이 ~1px면 눈으론 못 고른다). 보정값(`1.5px`)은 **폰트 메트릭 종속** — 폰트/크기 바뀌면 재측정.
- 외부 패키지를 격리 재현에 쓸 땐 **버전을 프로젝트와 일치**시키고(`package.json` 확인) `tar`는 **패키지별 개별 디렉토리로 추출**한다.
...
53. **정리·상한 로직은 "존재"가 아니라 "전 경로 호출"을 검증 + 이중 저장소는 기준 원장 하나로** — 정리 함수가 폴백 분기에만 걸려 있어 디스크가 무한 누적됐다. 정의가 아니라 **호출부**를 grep할 것. 저장소가 둘(DB·디스크)이면 각자 세지 말고 한쪽을 기준 원장으로, 나머지는 미러로. 단 **기준이 비었을 때 미러를 전량 삭제하지 않도록 가드 필수**(§44 사고와 동형). 상한 카운트는 확장자 화이트리스트로 좁히고(`.gitkeep` 삭제 방지), 파괴적 일괄 삭제는 dry-run → 복구 가능성(사본·해시) 확인 → 실행 (§75 권장 1·2·4·5번 참조).
54. **git 명령이 저장소와 무관하게 일괄 실패하면 툴체인/라이선스 게이트를 의심** — macOS 메이저 업그레이드는 Xcode 라이선스 동의(`sudo xcodebuild -license`)·TCC 권한을 리셋해 `/usr/bin/git`을 통째로 막는다. `git --version`까지 죽으면 저장소 진단을 멈추고 환경부터 볼 것. 업그레이드 직후 첫 세션은 git·python3·venv 생존 확인 (§76 권장 1·2번 참조).
55. **"서버에서 X가 일어난다"는 제보의 첫 명령은 `hostname`** — 작업 머신과 운영 머신이 다른 프로젝트에서, 로컬 저장소에 그럴듯한 운영 데이터가 있다는 이유로 엉뚱한 머신을 정리하고 "해결"로 보고한 사고. 서비스가 이 머신에서 실제로 도는지(포트·프로세스·LaunchAgent) 확인하고, 조치 결과는 **어느 머신 기준인지 명시**해 보고한다. 제보 수치와 실측 수치의 자릿수가 다르면 대상 오인부터 의심. 메모에 적힌 "불가능/미설정"(접근 권한·인증·툴체인)은 단정 전 1회 재시험 후 갱신 (§77 권장 1·2·3·4번 참조).
