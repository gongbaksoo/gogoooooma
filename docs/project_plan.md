...
**미구현 (후속)**: 마이비 전채널 복귀(롤오프 후 재측정) / AI 분석 모달(`daily_prompts.json` + 하드룰, 예외 0건이면 LLM 미호출) / 소급 정정 재판정·알림 / 예외 카드 '확인함' 버튼 / 쿠팡 사입 제외 기준 월 목표 반영(목표 파일 대기).
...
| POST | `/api/monthly-review/ai-analysis` | AI 매출 분석 실행 (Gemini) — 41~43회차 컨텍스트 확장(brand_focus/brand_products/주요상품 매출순) ⭐ NEW (28회차) |
