...
**거래처(계좌) 단위 A군 감시 + 별칭 (48회차 — 월리뷰 분석축 재검증 후 신 1순위)**
- 월 리뷰가 실제로 깊게 파는 대상은 **파트→채널→거래처(R열), 브랜드→상품**. 그중 거래처 축은 실측으로 완전히 검증됨: 심층 6곳(11번가·지마켓·CJ·제제스스·롯데마트·이마트)이 전부 A군 채널·밀도 95~100%라 거래처 단위 예외가 실제로 발화한다.
- **주요 거래처 감시 패널**: A군 채널 소속 거래처를 60일 순매출 상위 `TOP_VENDORS(=8)`곳 뽑아, 각 거래처의 당일 순매출 vs 같은 요일 8주 범위 위치를 상시 표시(채널 표와 동일 규약). **기본 접힘** — 판정일 58.7%가 예외 0건이므로 조용한 날 화면이 길어지지 않게. 예외 카드로 오른 거래처는 이 패널에서도 강조·연결(같은 데이터).
- **별칭(`ACCOUNT_ALIAS`)은 표시 전용**: `11st→11번가` / `이베이→지마켓` / `스팜(제제지크)→제제스스` / `스팜(쏭레브)→쏭스스` / `쿠팡→쿠팡(위탁)`(사입 `쿠팡(로켓)`과 구분) / `베이비빌리(주식회사 빌리지베이비)→베이비빌리`. **집계·매칭은 항상 R열 원본 exact**(프로젝트 규칙). 응답은 `entity`(원본)와 `entity_display`(별칭)를 함께 실어 화면은 별칭, 참조는 원본.
- **이름 함정 자동 회피**: 커버리지 저하의 근본 원인인 쿠팡 사입은 B군이라 이 패널에서 자동 제외 → 죽은 계좌(`십일번가풀필먼트`, 밀도 3%)·중단된 `쿠팡(로켓)`은 애초에 안 뜬다.
- 표본 부족(같은 요일 유효관측 < 6)인 거래처는 위치를 단정하지 않고 **'표본 부족'**으로 표기.
...
- **신 2순위 = 브랜드 일별 (A군 한정 재정의)** — 조건부. 마이비 순매출의 **67.9%가 배치**(쿠팡 사입 58.2%+다이소 9.7%), A군 28.8%뿐. 전 채널로 그리면 배치 스파이크(단일일 최대 9,620만)가 신호를 압도 → **반드시 A군 스코프**. A군 한정 시 같은요일 CV 마이비 1.68→1.26, 누비 1.47→0.58로 정화. 쏭레브만 B군 8%라 전채널 허용 가능. **대기**(쿠팡 롤오프 후 재측정).
- **신 3순위 = SKU 승격 2개 한정** — 원 "다수 승격"은 반박됨. 게이트50만+A군+유효관측6을 다 적용하면 상위 15개 중 **딱 2개**(얼룩제거제 플러스 신형 용기 `1006045`/리필 `1006046`). 매출 1·2위 구형은 쿠팡 96~97%라 일별 무의미. 판단 기준은 '밀도'가 아니라 'A군 값이 50만 게이트를 넘는가'. **대기**.
- **커버리지 실측**: 260615 기준 **39.2%**(prod 260714는 43.5%). 쿠팡 사입 전량 이탈 시 **69.7%**('약 70%' 어림 적중).

**미구현 (후속)**: 브랜드 일별 A군 스코프(신2순위, 롤오프 후) / SKU 2개 승격(신3순위) / AI 분석 모달(`daily_prompts.json` + 하드룰, 예외 0건이면 LLM 미호출) / 소급 정정 재판정·알림 / 예외 카드 '확인함' 버튼 / 쿠팡 사입 제외 기준 월 목표 반영(목표 파일 대기).
